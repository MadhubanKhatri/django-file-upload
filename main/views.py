...
# HOME page view where loggedin user can upload files
def home(request):
...
# You have to create an account before login and username, email must be unique. 
def signup(request):
...
# User must be loggedin before accessing HOME page
def signin(request):
...
# Only loggedin users can upload files on the server.
